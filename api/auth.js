  ...
  try {
    ...
    if (action === 'consume_limit') {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (data.status !== 'active' || (data.usedToday >= data.dailyLimit)) {
          data.totalInvalidInject = (data.totalInvalidInject || 0) + 1;
          transaction.update(userDocRef, { totalInvalidInject: data.totalInvalidInject });
          
          if (data.status !== 'active') rejectionReason = 'Your account has been disabled.';
          else rejectionReason = 'Your daily inject limit has been reached.';
        ...
        transaction.update(userDocRef, {
          ...
          totalInvalidInject: data.totalInvalidInject || 0
        });
